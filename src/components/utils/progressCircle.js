...
import Path from '../../../node_modules/react-native-svg-charts/src/animated-path.js'
    ...
    const arcs = pieSlices.map((slice, index) => (
      {
        ...
        ...slice,
        path: shape.arc()
          .outerRadius(outerDiameter / 2 - 10)  // Radius of the pie
            ...
            {arcs.map((shape, index) => {
              return (
                <Path
                  ...
                  strokeWidth='7'
                  stroke={index === 1 ? 'transparent': progressColor}
                />
              )
            })}
          </G>
